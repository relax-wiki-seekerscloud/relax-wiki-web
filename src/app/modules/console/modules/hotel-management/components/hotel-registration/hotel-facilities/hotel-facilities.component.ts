...
export class HotelFacilitiesComponent implements OnInit {
  ...
  submitForm() {
    // const selectedFacility = this.Facility.filter(item => {
    //   return this.hotelFacilityForm.value.facilitiesArray.includes(item.value);
    // });
    // const selectedService = this.Service.filter(item => {
    //   return this.hotelFacilityForm.value.servicesArray.includes(item.value);
    // });
